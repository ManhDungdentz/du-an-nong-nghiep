...
def process_data(file):
    ...
    for col in df.columns:
        if col != 'Thời gian':
            # Ép mọi thứ về số, xử lý dấu phẩy, khoảng trắng, xóa bỏ ký tự lạ
            df[col] = pd.to_numeric(df[col].astype(str).str.replace(',', '.').str.strip(), errors='coerce')
            
    # Gộp dữ liệu trùng giây
    df = df.groupby('Thời gian').mean(numeric_only=True).reset_index()

    # Tự động sửa đơn vị PH/Nhiệt độ nếu số quá lớn
    for col in df.columns:
        u_col = col.upper()
...
if uploaded_files:
    ...
    if not df.empty:
        # Lấy mốc thời gian thực tế của file để tránh lỗi trắng biểu đồ
        ...
        if not df_filtered.empty:
            # Lấy TẤT CẢ các cột có chứa số (loại bỏ cột STT, index)
            ...
            if num_cols:
                m_cols = st.columns(4)
                for i, col_name in enumerate(num_cols[:12]): # Hiện tối đa 12 cái
            ...
            # QUAN TRỌNG: Mặc định chọn tất cả các cột có trong file để vẽ ngay lập tức
            selected_metrics = c2.multiselect("Bấm vào đây để THÊM thông số vẽ:", 
                                              num_cols, 
            ...
            if selected_metrics:
                ...
                for m in selected_metrics:
                    ...
                    if not p_data.empty:
                        # Dùng chế độ nối điểm (connectgaps) để file lịch bị ngắt quãng vẫn vẽ được đường
                        fig.add_trace(go.Scatter(
                            x=p_data['Thời gian'], 
                            y=p_data[m], 
                            mode='lines+markers' if len(p_data) < 50 else 'lines',
                            name=m,
                            connectgaps=True,
                            line=dict(width=1.5)
                        ))
